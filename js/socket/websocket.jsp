...
//             		url = "ws://192.168.202.136:8088/smartmeeting/smart/websocket?token="+token; 
            		...
            		url = "wss://192.168.0.193:8081/notice/websocket?token="+token;  
            	}
            	websocket = new WebSocket(url,[],{devel: true, debug: true,server:"token="+token,transports: transports});
            }else{
            	url= "http://192.168.0.193:8081/notice/socketjs?token="+token;
            ...
            websocket.onclose = function (event) {
            	common(false);
                log("--关闭连接连接-----" + JSON.stringify(event));
            };
        }
        ...
        function updateTransport(transport) {
          transports = (transport == 'all') ?  null : [transport];
        }
